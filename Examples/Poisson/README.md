...
.
├── data/
│   ├── Training_Poisson.pt        # training dataset (to be downloaded)
...
```bash
python train_DiSOL_Poisson.py \
    --pt_path ./data/Training_Poisson.pt \
    --save_dir ./model_ckpt \
    --epochs 500 \
    --batch_size 200 \
    --lr 1e-4 \
...
```bash
python run_DiSOL_Poisson.py \
    --pt_path ./data/Training_Poisson.pt \
    --ckpt ./model_ckpt/ckpt_epoch_0500.pth \
    --model_cfg_json ./model_ckpt/model_cfg.json \
    --out_dir ./output_results \
    --vis_n 4 \
    --ood_pt_path ./data/OOD_Test_Poisson.pt \
    --use_mask_metrics \
    --apply_mask_to_pred \
