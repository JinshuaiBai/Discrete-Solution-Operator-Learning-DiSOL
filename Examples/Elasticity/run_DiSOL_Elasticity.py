"""
DiSOL testing / evaluation script (Elasticity dataset)
...
def load_model_cfg(model_cfg_json: Optional[str], ckpt: Optional[Dict[str, Any]], ModelCfg):
    ...
    # fallback: minimal safe defaults for Elasticity case
    cfg = dict(
        in_channels=5,
        out_channels=2,
        
        head_type="none",  # Skip FiLM fusion
...
# Optional visualization
    if args.vis_n > 0:
        ...
        try:
            ...
            n_out_channels = y.shape[1]  # number of output channels
            
            for i in range(B):
                mk_i = mask_bin[i, 0].detach().cpu().numpy().astype(np.float32)
                
                fig = plt.figure(figsize=(10, 3 * n_out_channels))
                
                for ch in range(n_out_channels):
                    gt_i = y[i, ch].detach().cpu().numpy()
                    pd_i = pred[i, ch].detach().cpu().numpy()
                    err_i = np.abs(pd_i - gt_i)
                    
                    ax1 = fig.add_subplot(n_out_channels, 4, ch * 4 + 1)
                    ax2 = fig.add_subplot(n_out_channels, 4, ch * 4 + 2)
                    ax3 = fig.add_subplot(n_out_channels, 4, ch * 4 + 3)
                    ax4 = fig.add_subplot(n_out_channels, 4, ch * 4 + 4)
                    
                    ch_label = f"u{['x', 'y', 'z'][ch]}" if ch < 3 else f"ch{ch}"
                    
                    ax1.set_title(f"GT ({ch_label})")
                    im1 = ax1.imshow(gt_i)
                    plt.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)
                    
                    ax2.set_title(f"Pred ({ch_label})")
                    im2 = ax2.imshow(pd_i)
                    plt.colorbar(im2, ax=ax2, fraction=0.046, pad=0.04)
                    
                    ax3.set_title(f"|Err| ({ch_label})")
    ...
    if args.ood_pt_path is not None and os.path.isfile(args.ood_pt_path):
        ...
        n_out_channels = ood_out.shape[1]
        ...
        for i in range(n_ood):
            ...
            for ch in range(n_out_channels):
                gt_i = ood_out_dev[i, ch].detach().cpu().numpy()
                pd_i = ood_pred[i, ch].detach().cpu().numpy()
                err_i = np.abs(pd_i - gt_i)
